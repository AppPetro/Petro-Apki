...
# Cała dotychczasowa logika aplikacji zostaje wewnątrz funkcji run()
def run():
    ...
    try:
        ...
        st.error(f"Nie znalazłem pliku `{REF_PACKAGING_FILE}`. Umieść go obok `excel_to_epp_app.py`.")
        st.stop()
    ...
    order_df = order_df.rename(columns={mapping["symbol"]: "Symbol", mapping["ilość"]:  "Ilość"})
    ...
    order_df = order_df.dropna(subset=["Ilość"]).copy()
    ...
    def round_pkg(row):
        ...
        if use_packages == "Tak" and pack > 1:
            ...
            if rem != 0:
                ...
                messages.append(f"Poprawiono +{corrected-qty} szt. przy EAN {row['Symbol']}")
                return corrected
        return qty
    ...
    merged["Stawka VAT"] = merged.apply(
        lambda r: 5.0 if r["Symbol"].rstrip(".") in vat5 else r["Stawka VAT"], axis=1
    )

    data_df = merged[[
        "Symbol","Nazwa","Ilość","Waga jednostkowa [kg]",
        "Waga całkowita [kg]","Cena netto","Stawka VAT"
    ]]
    ...
    for seq, (_, r) in enumerate(data_df.iterrows(), start=1):
        s = r["Symbol"]
        f = [str(seq), "1", f'"{s}"', *["0"]*4, *["0.0000"]*2, '"szt."',
    ...
    # --- 9) Przyciski i podsumowania ---
    st.markdown(f"**Łączna waga zamówienia:** {total_weight:.2f} kg")
    st.download_button(label="Pobierz plik EPP", data=epp_content.encode("cp1250"), file_name=fname, mime="text/plain")
